...
print_usage() {
    ...
    echo "  -f,  --filter openai|cloudflare 指定过滤器"
...
filter(){
    ...
    case $filter_type in
        "openai") message=$(echo "$response" | jq -r .choices[0].message) ;;
        "cloudflare") message=$(echo "$response"|jq -r .result.response) ;;
...
send_request() {
    ...
    case $debug in
        ...
        1) echo "$response" | jq -c .  ;; # 只输出内容，使用 -r 选项以避免引号
        *) ;;
        
    esac
    response_time=$(( (end_time - start_time) / 1000000 ))  # 将纳秒转换为毫秒
...
showtime(){
    ...
    if [ $test == true ];then
        case $response_time in
            [0-2000])
                echo -e "\e[32m用时：$response_time ms\e[0m"
            ;;
            [3000-5000])
                echo -e "\e[33m用时：$response_time ms\e[0m"
            ;;
            *)
                echo -e "\e[31m用时：$response_time ms\e[0m"
            ;;
        esac
